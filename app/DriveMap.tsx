...
import type { DriveData, DrivePoint } from "../lib/drive-data";
...
export default function DriveMap({
  ...
  useEffect(() => {
    ...
    for (const [trip, route] of routes) {
      ...
      L.polyline(coords, {
        ...
        opacity: isSelected ? 0.16 : isDimmed ? 0.025 : 0.08,
        lineCap: "round",
        lineJoin: "round",
        interactive: false,
      }).addTo(group);
      L.polyline(coords, {
        renderer: canvas,
        color: "#0d6efd",
        weight: isSelected ? 4 : 2.25,
        opacity: isSelected ? 0.9 : isDimmed ? 0.09 : 0.35,
        lineCap: "round",
        lineJoin: "round",
        interactive: false,
      }).addTo(group);
    ...
    if (bounds.length)
      map.fitBounds(L.latLngBounds(bounds), {
        padding: [34, 34],
        maxZoom: selectedBounds.length ? 15 : 12,
      });
  ...
  return (
    <div
      ...
      aria-label="Interactive map of recorded drives"
    />
  );
}
